...
namespace FOVFix
{
    // Ported note: targeted an obfuscated static generic helper (GClass3380.CloneItem) from the
    // original 4.0 build. No equivalent found in the live 4.1.3 assembly by signature search
    // (not even the author was sure of its purpose - "don't remember its purpose..."), and it only
    // affects a niche case (weapon scroll-offset persistence while testing a weapon in the hideout).
    // Dropped rather than guessed at - see README_PORT_NOTES.txt.

    // Ported note: CameraUpdatePatch, OpticPanelPatch and ScopeZoomPatch (targeting obfuscated
    // OpticComponentUpdater/ScopeZoomHandler internals from the original build) were never actually
    // enabled in Plugin.Awake() in the upstream source either - dead code removed rather than ported.
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public class FovValuePatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            // Ported note: original targeted a top-level obfuscated "Class1841.method_0". In the live
            // 4.1.3 assembly the same clamp-lambda lives as a nested compiler-generated closure class
            // on GameSettingsGroup itself (GameSettingsGroup+CG_Ctor), method name unchanged
            // (method_0(int x) => Mathf.Clamp(x, 50, 75)) - confirmed by decompiling GameSettingsGroup's
            // constructor directly, not guessed.
            return AccessTools.Method(AccessTools.Inner(typeof(GameSettingsGroup), "CG_Ctor"), "method_0");
        }
    ...
    public class LerpCameraPatch : ModulePatch
    {
            ...
            Transform ____bone0, Transform ____bone1)
        {
            ...
            if (player != null && player.IsYourPlayer && firearmController.Weapon != null)
            {
                ...
                // Ported note: original read obfuscated "Boolean_0". Live 4.1.3's ProceduralWeaponAnimation
                // exposes the same left-stance flag as public property "LeftStance" (backed by field
                // _isLeftStance) - confirmed by decompiling ProceduralWeaponAnimation directly.
                bool isDoingLeftShoulder = isRealLeftShoulder || __instance.LeftStance;  //detects if in BSG's left stance
                ...
                // Ported note: original read obfuscated "SharedGameSettingsClass". Live 4.1.3 has this
                // exact Singleton<SettingsManager>.Instance.Game.Settings.{X} access pattern already in
                // use elsewhere in the engine's own code (e.g. CameraManager), with the same Game/Control
                // nested structure - confirmed by decompiling SettingsManager directly.
                ...
                // Ported note: original also called obfuscated "__instance.method_19(dt)" here. No
                // confident 4.1.3 equivalent found - several public void(float dt) candidates exist on
                // ProceduralWeaponAnimation (BlendAnimatorPose, ApplyComplexRotation, LateTransformations,
                // etc.) with no signature-level way to disambiguate which one it was without in-game
                // testing. Dropped rather than guessed at; camera positioning itself (the actual point of
                // this patch) is fully handled by the rest of this prefix body above.
    ...
    public class PwaWeaponParamsPatch : ModulePatch
    {
        ...
        protected override MethodBase GetTargetMethod()
        {
            ...
            // Ported note: original targeted obfuscated "method_23". In the live 4.1.3 assembly
            // InitWeaponData(...) is the method that actually sets the _firearmController field
            // (confirmed by decompiling ProceduralWeaponAnimation - "_firearmController = firearms as
            // Player.FirearmController;" is its first line), matching the postfix's assumption that
            // the field is already populated by the time it runs. Postfix ignores the target's own
            // parameters (none declared here beyond __instance), so the extra InitWeaponData args
            // don't need to be bound.
            return AccessTools.Method(typeof(EFT.Animations.ProceduralWeaponAnimation), "InitWeaponData");
        }
    ...
    // THE FIX for the crouch-FOV bug. ProceduralWeaponAnimation.OnAimOrPoseChanged is vanilla engine
    // code that fires on every Pose change (crouch/stand) as well as on aim start/stop, and it calls
    // CameraManager.SetFov(x, 1f, ...) itself with x = 35f for optics (a hardcoded vanilla value,
    // ignoring our mod's magnification/toggle-zoom multipliers entirely) - confirmed via debug log
    // stack trace: OnAimOrPoseChanged -> set_Pose -> CG_InitHandsContainer -> SetPoseLevel ->
    // MovementState.ChangePose -> Player.TogglePose (the crouch key). Our FovController only
    // recalculates zoom when ADSWatcher/ScopeFOVWatcher/etc. actually change value - they don't
    // change on a pose transition while already aiming, so this vanilla reset sticks until the next
    // real ADS toggle. Fix: re-run our own FOV calc right after, overwriting whatever vanilla just set.
    public class OnAimOrPoseChangedFixPatch : ModulePatch
    {
        private static FieldInfo _fcField;
        private static FieldInfo _playerField;
        ...
        protected override MethodBase GetTargetMethod()
        {
            _fcField = AccessTools.Field(typeof(ProceduralWeaponAnimation), "_firearmController");
            _playerField = AccessTools.Field(typeof(FirearmController), "_player");
            return AccessTools.Method(typeof(ProceduralWeaponAnimation), "OnAimOrPoseChanged");
        }
