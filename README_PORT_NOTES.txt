...
ИСПРАВЛЕННЫЙ БАГ #2 (найден пользователем после публикации): заморозка позы
оружия/рук на несколько секунд при ADS в тире хидаута
-------------------------------------------------------------------------------
Симптом (видео пользователя, разобрано покадрово через ffmpeg): в тестовом
тире хидаута при попытке прицелиться поза оружия/рук полностью замирает на
5-9 секунд под неестественным углом (ствол уходит вбок), затем сама
"размораживается" одним скачком; по словам пользователя, накопленные
выстрелы тоже вылетают разом в этот момент.

Причина: `FovController.ScrollCameraOffset` (getter) в хидауте вызывает
`WeaponClassExtension.GetCustomProperty(CurrentWeapon)`, которая возвращает
null для тестового оружия из тира — потому что `CloneItemPatch` (единственный
код, который метил клонированное тестовое оружие через SetCustomProperty)
был выброшен при портировании на 4.1.3 (не нашли эквивалент обфусцированного
`GClass3380.CloneItem`, см. ниже). `Dictionary<string,float>.TryGetValue(null, ...)`
бросает `ArgumentNullException` на null-ключе. Этот getter вызывается из
`LerpCameraPatch.Prefix` КАЖДЫЙ кадр, пока держишь ADS
(`camZ = __instance.IsAiming ? camZ + Plugin.FovController.ScrollCameraOffset : camZ`)
— исключение там замораживает весь остаток метода, включая обновление
позиции камеры/рук, ровно на всё время удержания прицела. Отпустил ПКМ —
`IsAiming` стал false, getter больше не дёргается, исключения прекращаются,
поза "размораживается" одним кадром.

Фикс (`FovController.cs`, `ScrollCameraOffset` getter): явная проверка
`if (id == null) return 0f;` перед обращением к словарю. Сеттер уже был
null-safe через `IsNullOrWhiteSpace()`. Это НЕ требует возвращать
`CloneItemPatch` — просто делает геттер устойчивым к некликонированному
(немеченому) оружию, чем оно, по сути, и является в тестовом тире.
