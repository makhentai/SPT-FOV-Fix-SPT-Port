...
namespace FOVFix
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            // CloneItemPatch dropped - see FovPatches.cs ported note.
            new SetPlayerAimingPatch().Enable();
            new CalculateScaleValueByFovPatch().Enable();
            new OnAimOrPoseChangedFixPatch().Enable(); // THE crouch-FOV-bug fix - see ported note.
        }
        ...
        private void CheckForMods()
        {
            ...
            {
                _detectedMods = true;
                // Ported note: original checked GUID "RealismMod". Our RealismMod port
                // (RealismModPort\port) is registered under "com.realismmod.ballistics.port" -
                // see RealismModPort\README_PORT_NOTES.txt. Checking the original GUID too in case
                // the unmodified upstream RealismMod is ever installed side by side.
                if (Chainloader.PluginInfos.ContainsKey("com.realismmod.ballistics.port") || Chainloader.PluginInfos.ContainsKey("RealismMod"))
                {
